<div class="mt-4 p-10 bg-white dark:bg-[#2d3748] rounded-lg shadow-xl w-full animate-in fade-in duration-300 border border-gray-200 dark:border-transparent">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-10 items-start">
     ...
     x-data="{ 
        ...
        handleFiles(files) {
            ...
            if (ext !== 'txt' && ext !== 'csv') {
                alert('Por favor, selecione apenas arquivos .txt ou .csv');
                return;
            }
            ...
            reader.onload = (e) => {
                $wire.set('importText', e.target.result);
            };
            reader.readAsText(file);
        }
     }">
    
    <label class="text-[0.9rem] font-bold text-gray-500 dark:text-[#a0aec0] mb-2 uppercase">Arquivo .txt ou .csv</label>
    ...
    <div class="flex gap-3 items-center relative"
        ...
        {{-- Input de texto: Mantive o estilo, adicionei borda de destaque no drag --}}
        <div class="relative flex-grow">
            <input type="text" 
                class="w-full bg-gray-100 dark:bg-[#4a5568] border-2 px-4 py-3 rounded-md text-sm outline-none transition-all duration-200" 
                :class="isDropping ? 'border-[#4a90e2] border-dashed ring-2 ring-blue-500/20' : 'border-gray-300 dark:border-[#4a5568] border-solid'"
                placeholder="Escolher ou Arrastar Arquivo" 
                x-model="fileName"
                readonly>
        </div>
        
        {{-- BOTÃO PROCURAR: Visual 100% Original --}}
        <label class="bg-[#4a90e2] hover:bg-[#3a7bd5] text-white px-6 py-3 rounded-md font-semibold text-xs uppercase cursor-pointer transition-colors whitespace-nowrap">
            ...
            <button type="button" 
                    ...
                    class="w-fit bg-[#4a90e2] hover:bg-[#3a7bd5] text-white px-6 py-3.5 rounded-md font-bold text-sm uppercase transition-all shadow-md flex items-center gap-2">
                <span wire:loading.remove wire:target="processImport">Importar Arquivo</span>
                <span wire:loading wire:target="processImport" class="flex items-center gap-2">
                    <i class="ph ph-circle-notch animate-spin"></i> Processando...
                </span>
            </button>
        </div>
