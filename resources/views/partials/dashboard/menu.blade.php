            ...
            @foreach([
                ...
                <div class="relative">
                    ...
                    <div x-show="openMenu === '{{ $key }}'" @click.away="openMenu = null" x-transition class="absolute top-[48px] left-0 w-52 bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-white/10 shadow-2xl rounded-b-lg py-2 z-[110]" style="display: none;">
                        ...
                        @elseif($key === 'layout')
                            <a href="{{ route('store.dashboard.layout.visual-identity', ['slug' => auth('store_user')->user()->store->url_slug]) }}" class="block px-6 py-2 text-xs font-semibold text-zinc-500 dark:text-zinc-400">Identidade Visual</a>
